...
module.exports.sendOtp = async (req, res) => {
  try {
    ...
    if (parts.length !== 2 || !parts[1].toLowerCase().includes(".edu")) {
      return res.status(400).json({ success: false, message: "Please use your college email with a .edu domain" });
    }
...
module.exports.verifyOtp = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "OTP verification failed. Try again." });
  }
};
...
module.exports.register = async (req, res) => {
  try {
    const {
      name, email, password, role, college, department, batchYear,
      company, jobTitle, bio, hourlyRate, availability, skills, interests,
    } = req.body;
    ...
    if (role === "student") {
      ...
      await new StudentProfile({
        userId: existingUser._id, department, batchYear,
        interests: parsedInterests, profileImage,
      }).save();
    }
    ...
    if (role === "alumni") {
      ...
      await new AlumniProfile({
        userId: existingUser._id, department, batchYear,
        company, jobTitle, bio, hourlyRate, availability,
        skills: parsedSkills, profileImage,
      }).save();
    }
    ...
    return res.status(201).json({
      ...
      token,
      user: { id: existingUser._id, name: existingUser.name, role: existingUser.role },
    });
  } catch (err) {
