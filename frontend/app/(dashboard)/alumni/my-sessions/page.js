...
function MySessionsPageInner() {
  ...
  const handleStartSession = async () => {
    if (!meetLinkInput.trim()) {
      showToast("error", "Please enter a meet link");
      return;
    }
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${API_BASE}/alumni/sessions/${startModal._id}/start`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (data.success) {
        setSessions((prev) =>
          prev.map((s) =>
            s._id === startModal._id
              ? { ...s, status: "live", meetLink: meetLinkInput.trim() }
              : s,
          ),
        );
        setStartModal(null);
        ...
        showToast(
          "success",
          "Session is now live! Students have been notified.",
        );
      } else {
  ...
  const handleEndSession = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${API_BASE}/alumni/sessions/${endConfirm._id}/end`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      if (data.success) {
        setSessions((prev) =>
          prev.map((s) =>
            s._id === endConfirm._id
              ? { ...s, status: "completed", meetLink: null }
              : s,
          ),
        );
        setEndConfirm(null);
  ...
  const handleViewReviews = async (session) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/student/sessions/${session._id}/reviews`,
      );
      const data = await res.json();
          ...
          {displaySessions.map((session) => (
            ...
            >
              ...
              <div className="session-body">
                ...
                <div className="session-meta">
                  <span className="meta-item">
                    <DollarSign size={14} />₹{session.price || 0}
                  </span>
                </div>
                ...
                <div className="card-actions">
                  <button
                    className="btn-track"
                    onClick={() =>
                      router.push(`/alumni/session/${session._id}/participants`)
                    }
                  >
                  ...
                  {session.status === "completed" && (
                    <button
                      ...
                      onClick={() => handleViewReviews(session)}
                    >
        ...
        <div className="modal-overlay" onClick={() => setStartModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="start-modal-desc">
              Paste your Google Meet / Zoom link below. Students will be
      ...
      {endConfirm && (
        <div className="modal-overlay" onClick={() => setEndConfirm(null)}>
          <div className="confirm-dialog" onClick={(e) => e.stopPropagation()}>
            <h3>End Session?</h3>
            <p>
              This will mark the session as completed and remove the meet link.
              Students will be notified and can leave reviews.
            </p>
            <div className="modal-actions">
              <button
                className="btn-cancel"
                ...
                <div className="reviews-aggregate">
                  <div className="reviews-avg">
                    ...
                    <span className="reviews-avg-num">
                      {(
                        reviewsModal.list.reduce((s, r) => s + r.rating, 0) /
                        reviewsModal.list.length
                      ).toFixed(1)}
                    </span>
                    <span className="reviews-avg-label">
                      avg · {reviewsModal.list.length} review
                      {reviewsModal.list.length !== 1 ? "s" : ""}
                    </span>
                  </div>
                </div>
                    ...
                    <div key={r._id} className="alumni-review-card">
                      <div className="alumni-review-top">
                        <div className="alumni-reviewer-info">
                          ...
                          <span className="alumni-reviewer-name">
                      ...
                      {r.comment && (
                        <p className="alumni-review-comment">{r.comment}</p>
                      )}
